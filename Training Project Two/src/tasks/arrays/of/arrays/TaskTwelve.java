...
//        printSortArray(sortDecArray(array));
    ...
    private static double[][] sortIncArray(double[][] array){            //Метод сортировки массива по возрастанию
        for(int column = 0; column < array.length; column++){           //с помощью рекурсии
            for(int line = 1;line < array[column].length;line++){
                if(array[column][line-1] > array[column][line]){
                    double buf = array[column][line-1];
                    array[column][line-1] = array[column][line];
